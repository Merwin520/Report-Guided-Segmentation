
import torch
import torch.nn as nn
import torch.nn.functional as F
from typing import Dict, Optional, List
import numpy as np
from scipy.ndimage import distance_transform_edt


class GPUBoundaryLoss(nn.Module):
    """
    GPU优化的Boundary Loss
    基于距离变换图的边界损失，替代Hausdorff距离
    统一规范：内部处理sigmoid，避免double sigmoid问题
    """
    
    def __init__(self, 
                 reduction: str = 'mean',
                 alpha: float = 1.0,
                 normalize_by_image_size: bool = True):
        """
        Args:
            reduction: 损失归约方式 ('mean', 'sum', 'none')
            alpha: 边界损失权重因子
            normalize_by_image_size: 是否按图像尺寸归一化
        """
        super().__init__()
        self.reduction = reduction
        self.alpha = alpha
        self.normalize_by_image_size = normalize_by_image_size
    
    def compute_distance_transform(self, mask: torch.Tensor) -> torch.Tensor:
        """
        计算距离变换图
        
        Args:
            mask: [B, H, W] 二值mask
            
        Returns:
            distance_maps: [B, H, W] 距离变换图
        """
        batch_size, H, W = mask.shape
        distance_maps = torch.zeros_like(mask, dtype=torch.float32)
        
        # 对每个batch单独处理
        for b in range(batch_size):
            mask_np = mask[b].detach().cpu().numpy()
            
            # 计算前景和背景的距离变换
            # 前景到背景边界的距离（内部距离为负）
            dt_fg = distance_transform_edt(mask_np)
            # 背景到前景边界的距离（外部距离为正）
            dt_bg = distance_transform_edt(1 - mask_np)
            
            # 合并：前景内部为负距离，背景为正距离
            distance_map = np.where(mask_np > 0.5, -dt_fg, dt_bg)
            
            distance_maps[b] = torch.from_numpy(distance_map).to(mask.device)
        
        return distance_maps
    
    def forward(self, pred_logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        计算Boundary Loss
        
        Args:
            pred_logits: [B, H, W] 预测logits (未经sigmoid)
            target: [B, H, W] 真实mask (0-1)
            
        Returns:
            boundary_loss: 标量损失
        """
        # 统一规范：在loss内部执行sigmoid
        pred_probs = torch.sigmoid(pred_logits)
        
        # 获取图像尺寸
        batch_size, H, W = pred_logits.shape
        
        # 计算目标mask的距离变换图
        target_binary = (target > 0.5).float()
        distance_maps = self.compute_distance_transform(target_binary)
        
        # Boundary Loss: 预测概率与距离变换图的加权乘积
        # 距离越远，惩罚越大
        boundary_loss = pred_probs * distance_maps
        
        # 归一化处理
        if self.normalize_by_image_size:
            # 按图像对角线长度归一化
            diagonal_length = torch.sqrt(torch.tensor(H*H + W*W, dtype=torch.float32, device=pred_logits.device))
            boundary_loss = boundary_loss / diagonal_length
        
        # 取绝对值并应用alpha权重
        boundary_loss = self.alpha * torch.abs(boundary_loss)
        
        # 归约
        if self.reduction == 'mean':
            return boundary_loss.mean()
        elif self.reduction == 'sum':
            return boundary_loss.sum()
        else:
            return boundary_loss


class UnifiedDiceLoss(nn.Module):
    """
    统一的Dice损失 - 内部处理sigmoid
    """
    
    def __init__(self, smooth: float = 1e-5):
        super().__init__()
        self.smooth = smooth
    
    def forward(self, pred_logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        Args:
            pred_logits: [B, H, W] 预测logits (未经sigmoid)
            target: [B, H, W] 真实标签 (0-1)
        """
        # 统一规范：在loss内部执行sigmoid
        pred_probs = torch.sigmoid(pred_logits)
        
        batch_size = pred_logits.size(0)
        pred_flat = pred_probs.view(batch_size, -1)
        target_flat = target.view(batch_size, -1)
        
        intersection = (pred_flat * target_flat).sum(dim=1)
        pred_sum = pred_flat.sum(dim=1)
        target_sum = target_flat.sum(dim=1)
        
        dice_coeff = (2 * intersection + self.smooth) / (pred_sum + target_sum + self.smooth)
        return 1 - dice_coeff.mean()


class UnifiedFocalLoss(nn.Module):
    """
    统一的Focal损失 - 内部处理sigmoid
    """
    
    def __init__(self, alpha: float = 1, gamma: float = 2, reduction: str = 'mean'):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
        self.reduction = reduction
    
    def forward(self, pred_logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        Args:
            pred_logits: [B, H, W] 预测logits (未经sigmoid)
            target: [B, H, W] 真实标签 (0-1)
        """
        # 统一规范：在loss内部执行sigmoid
        pred_probs = torch.sigmoid(pred_logits)
        
        # 计算BCE
        bce_loss = F.binary_cross_entropy(pred_probs, target, reduction='none')
        
        # 计算focal权重
        p_t = pred_probs * target + (1 - pred_probs) * (1 - target)
        focal_weight = self.alpha * (1 - p_t) ** self.gamma
        
        # Focal损失
        focal_loss = focal_weight * bce_loss
        
        if self.reduction == 'mean':
            return focal_loss.mean()
        elif self.reduction == 'sum':
            return focal_loss.sum()
        else:
            return focal_loss


class UnifiedTripleLoss(nn.Module):
    """
    统一的三重损失函数 (Dice + Focal + Boundary)
    完全清晰的参数命名，不混淆
    """
    
    def __init__(self, 
                 dice_weight: float = 0.3,
                 focal_weight: float = 0.3,
                 boundary_weight: float = 0.4,  # 保持原参数名兼容性
                 dice_smooth: float = 1e-5,
                 focal_alpha: float = 1.0,
                 focal_gamma: float = 2.0,
                 boundary_alpha: float = 1.0,  # 替代hausdorff_sample_points等参数
                 normalize_boundary: bool = True,  # 替代normalize_hausdorff
                 use_dynamic_weights: bool = False,
                 total_epochs: int = 30,
                 weight_stages: List[Dict] = None,
                 # 向后兼容的参数名
                 hausdorff_weight: Optional[float] = None,
                 hausdorff_sample_points: Optional[int] = None,
                 normalize_hausdorff: Optional[bool] = None):
        """
        Args:
            dice_weight: Dice损失权重
            focal_weight: Focal损失权重
            boundary_weight: Boundary损失权重 (原hausdorff_weight)
            dice_smooth: Dice损失平滑因子
            focal_alpha: Focal损失alpha参数
            focal_gamma: Focal损失gamma参数
            boundary_alpha: Boundary损失权重因子 (替代hausdorff相关参数)
            normalize_boundary: 是否归一化Boundary损失 (替代normalize_hausdorff)
            use_dynamic_weights: 是否使用动态权重
            total_epochs: 总训练轮数
            weight_stages: 权重阶段配置
            
            # 向后兼容参数（会自动转换）
            hausdorff_weight: 向后兼容，等同于boundary_weight
            hausdorff_sample_points: 向后兼容，映射到boundary_alpha
            normalize_hausdorff: 向后兼容，等同于normalize_boundary
        """
        super().__init__()
        
        # 向后兼容处理
        if hausdorff_weight is not None:
            boundary_weight = hausdorff_weight
        if normalize_hausdorff is not None:
            normalize_boundary = normalize_hausdorff
        if hausdorff_sample_points is not None:
            # 将sample_points映射为boundary_alpha的合理值
            boundary_alpha = min(2.0, max(0.5, hausdorff_sample_points / 1000.0))
        
        self.dice_weight = dice_weight
        self.focal_weight = focal_weight
        self.boundary_weight = boundary_weight
        
        # 动态权重配置
        self.use_dynamic_weights = use_dynamic_weights
        
        if self.use_dynamic_weights:
            # 默认权重阶段 (更新键名为boundary)
            if weight_stages is None:
                weight_stages = [
                    {'progress_percent': 30, 'weights': {'dice': 0.6, 'focal': 0.3, 'boundary': 0.1}},
                    {'progress_percent': 70, 'weights': {'dice': 0.4, 'focal': 0.3, 'boundary': 0.3}},
                    {'progress_percent': 100, 'weights': {'dice': 0.3, 'focal': 0.3, 'boundary': 0.4}}
                ]
            else:
                # 兼容处理：将hausdorff键转换为boundary键
                for stage in weight_stages:
                    if 'hausdorff' in stage['weights'] and 'boundary' not in stage['weights']:
                        stage['weights']['boundary'] = stage['weights']['hausdorff']
                        del stage['weights']['hausdorff']
            
            self.weight_scheduler = SimpleProgressWeightScheduler(weight_stages, total_epochs)
            self.current_weights = self.weight_scheduler.get_weights(0)
        else:
            self.current_weights = {'dice': dice_weight, 'focal': focal_weight, 'boundary': boundary_weight}
        
        # 初始化各个损失组件
        self.dice_loss_fn = UnifiedDiceLoss(smooth=dice_smooth)
        self.focal_loss_fn = UnifiedFocalLoss(alpha=focal_alpha, gamma=focal_gamma)
        self.boundary_loss_fn = GPUBoundaryLoss(
            alpha=boundary_alpha,
            normalize_by_image_size=normalize_boundary
        )
        
        print(f"Unified Triple Loss (Focal + Dice + Boundary) initialized:")
        print(f"  - Use dynamic weights: {use_dynamic_weights}")
        if use_dynamic_weights:
            print(f"  - Dynamic weight stages configured")
        else:
            print(f"  - Dice weight: {dice_weight}")
            print(f"  - Focal weight: {focal_weight}")
            print(f"  - Boundary weight: {boundary_weight}")
        print(f"  - Input format: logits (no external sigmoid needed)")
        if hausdorff_weight is not None:
            print(f"  - Note: hausdorff_weight automatically mapped to boundary_weight")
    
    def update_epoch(self, current_epoch: int):
        """更新当前epoch，用于动态权重计算"""
        if self.use_dynamic_weights:
            self.current_weights = self.weight_scheduler.get_weights(current_epoch)
    
    def get_current_weights(self) -> Dict[str, float]:
        """获取当前权重"""
        return self.current_weights.copy()
    
    def forward(self, pred_logits: torch.Tensor, target: torch.Tensor, 
                return_components: bool = False) -> torch.Tensor:
        """
        前向传播
        
        Args:
            pred_logits: [B, H, W] 预测logits (未经sigmoid)
            target: [B, H, W] 真实标签 (0-1)
            return_components: 是否返回各组件损失
            
        Returns:
            total_loss: 标量损失 或 损失字典
        """
        # 计算各个损失组件（都内部处理sigmoid）
        dice_loss = self.dice_loss_fn(pred_logits, target)
        focal_loss = self.focal_loss_fn(pred_logits, target)
        boundary_loss = self.boundary_loss_fn(pred_logits, target)
        
        # 使用当前权重计算总损失
        weights = self.current_weights
        total_loss = (weights['dice'] * dice_loss + 
                     weights['focal'] * focal_loss + 
                     weights['boundary'] * boundary_loss)
        
        if return_components:
            return {
                'total_loss': total_loss,
                'dice_loss': dice_loss,
                'focal_loss': focal_loss,
                'boundary_loss': boundary_loss,
                'current_weights': self.current_weights.copy()
            }
        else:
            return total_loss


class SimpleProgressWeightScheduler:
    """
    简单的训练进度权重调度器 - 基于训练百分比
    """
    
    def __init__(self, weight_stages: List[Dict] = None, total_epochs: int = 30):
        """
        Args:
            weight_stages: 权重阶段配置列表
            total_epochs: 总训练轮数
        """
        self.total_epochs = total_epochs
        
        # 默认权重阶段 (使用boundary替代hausdorff)
        if weight_stages is None:
            self.weight_stages = [
                {'progress_percent': 30, 'weights': {'dice': 0.6, 'focal': 0.3, 'boundary': 0.1}},
                {'progress_percent': 70, 'weights': {'dice': 0.4, 'focal': 0.3, 'boundary': 0.3}},
                {'progress_percent': 100, 'weights': {'dice': 0.3, 'focal': 0.3, 'boundary': 0.4}}
            ]
        else:
            self.weight_stages = weight_stages
        
        print(f"Simple Progress Weight Scheduler initialized:")
        for i, stage in enumerate(self.weight_stages):
            start = self.weight_stages[i-1]['progress_percent'] if i > 0 else 0
            print(f"  Stage {i+1}: {start}%-{stage['progress_percent']}% -> {stage['weights']}")
    
    def get_weights(self, current_epoch: int) -> Dict[str, float]:
        """根据当前epoch获取权重"""
        progress = (current_epoch / self.total_epochs) * 100
        
        # 找到当前进度对应的权重阶段
        for stage in self.weight_stages:
            if progress <= stage['progress_percent']:
                return stage['weights']
        
        # 如果超出所有阶段，返回最后一个阶段的权重
        return self.weight_stages[-1]['weights']

def create_unified_loss(cfg) -> nn.Module:
    """
    根据配置创建统一的损失函数
    简化版本，直接使用新的boundary参数
    """
    dice_weight = getattr(cfg, 'dice_weight', 0.3)
    focal_weight = getattr(cfg, 'focal_weight', 0.3)
    boundary_weight = getattr(cfg, 'boundary_weight', 0.4)
    
    dice_smooth = getattr(cfg, 'dice_smooth', 1e-5)
    focal_alpha = getattr(cfg, 'focal_alpha', 1.0)
    focal_gamma = getattr(cfg, 'focal_gamma', 2.0)
    
    boundary_alpha = getattr(cfg, 'boundary_alpha', 1.0)
    normalize_boundary = getattr(cfg, 'normalize_boundary', True)
    
    # 动态权重参数
    use_dynamic_weights = getattr(cfg, 'use_dynamic_weights', False)
    total_epochs = getattr(cfg, 'total_epochs', 30)
    weight_stages = getattr(cfg, 'weight_stages', None)
    
    return UnifiedTripleLoss(
        dice_weight=dice_weight,
        focal_weight=focal_weight,
        boundary_weight=boundary_weight,
        dice_smooth=dice_smooth,
        focal_alpha=focal_alpha,
        focal_gamma=focal_gamma,
        boundary_alpha=boundary_alpha,
        normalize_boundary=normalize_boundary,
        use_dynamic_weights=use_dynamic_weights,
        total_epochs=total_epochs,
        weight_stages=weight_stages
    )


# 全局缓存，避免重复创建损失函数
_loss_function_cache = {}

def compute_integrated_loss(pred_logits: torch.Tensor, 
                          gt_masks: torch.Tensor, 
                          cfg) -> torch.Tensor:
    """
    统一的损失计算接口
    使用缓存避免重复创建损失函数
    简化版本，使用Boundary Loss
    """
    device = pred_logits.device
    debug_loss = getattr(cfg, 'debug_loss_components', False)
    
    # 检查是否启用动态权重
    use_dynamic_weights = getattr(cfg, 'use_dynamic_weights', False)
    
    # 创建缓存key
    cache_key = f"{device}_{use_dynamic_weights}"
    
    if use_dynamic_weights:
        # 使用动态权重损失函数（使用缓存）
        if cache_key not in _loss_function_cache:
            _loss_function_cache[cache_key] = create_unified_loss(cfg).to(device)
        
        loss_fn = _loss_function_cache[cache_key]
        total_loss = loss_fn(pred_logits, gt_masks)
        
        # 调试信息
        if debug_loss:
            with torch.no_grad():
                components = loss_fn(pred_logits, gt_masks, return_components=True)
                weights = components.get('current_weights', {})
                print(f"DYNAMIC LOSS (Focal+Dice+Boundary) - Dice: {components['dice_loss'].item():.4f} (w={weights.get('dice', 0):.3f}), "
                      f"Focal: {components['focal_loss'].item():.4f} (w={weights.get('focal', 0):.3f}), "
                      f"Boundary: {components['boundary_loss'].item():.4f} (w={weights.get('boundary', 0):.3f}), "
                      f"Total: {components['total_loss'].item():.4f}")
        
        return total_loss
    
    else:
        # 静态权重实现
        dice_weight = getattr(cfg, 'dice_weight', 0.3)
        focal_weight = getattr(cfg, 'focal_weight', 0.3)
        boundary_weight = getattr(cfg, 'boundary_weight', 0.4)
            
        dice_smooth = getattr(cfg, 'dice_smooth', 1e-5)
        focal_alpha = getattr(cfg, 'focal_alpha', 1.0)
        focal_gamma = getattr(cfg, 'focal_gamma', 2.0)
        
        boundary_alpha = getattr(cfg, 'boundary_alpha', 1.0)
        normalize_boundary = getattr(cfg, 'normalize_boundary', True)
        
        pred_probs = torch.sigmoid(pred_logits)
        
        # Dice损失
        batch_size = pred_logits.size(0)
        pred_flat = pred_probs.view(batch_size, -1)
        target_flat = gt_masks.view(batch_size, -1)
        
        intersection = (pred_flat * target_flat).sum(dim=1)
        dice_coeff = (2 * intersection + dice_smooth) / (
            pred_flat.sum(dim=1) + target_flat.sum(dim=1) + dice_smooth
        )
        dice_loss = 1 - dice_coeff.mean()
        
        # Focal损失
        bce_loss = F.binary_cross_entropy(pred_probs, gt_masks, reduction='none')
        p_t = pred_probs * gt_masks + (1 - pred_probs) * (1 - gt_masks)
        focal_weight_map = focal_alpha * (1 - p_t) ** focal_gamma
        focal_loss = (focal_weight_map * bce_loss).mean()
        
        # Boundary损失（使用缓存）
        boundary_cache_key = f"boundary_{device}_{boundary_alpha}_{normalize_boundary}"
        if boundary_cache_key not in _loss_function_cache:
            _loss_function_cache[boundary_cache_key] = GPUBoundaryLoss(
                alpha=boundary_alpha,
                normalize_by_image_size=normalize_boundary
            ).to(device)
        
        boundary_loss_fn = _loss_function_cache[boundary_cache_key]
        boundary_loss = boundary_loss_fn(pred_logits, gt_masks)
        
        # 总损失
        total_loss = (dice_weight * dice_loss + 
                     focal_weight * focal_loss + 
                     boundary_weight * boundary_loss)
        
        # 调试信息
        if debug_loss:
            print(f"UNIFIED LOSS (Focal+Dice+Boundary) - Dice: {dice_loss.item():.4f}, Focal: {focal_loss.item():.4f}, "
                  f"Boundary: {boundary_loss.item():.4f}, Total: {total_loss.item():.4f}")
        
        return total_loss


# 如果你需要向后兼容旧的配置文件，可以使用这个函数
def create_unified_loss_with_compatibility(cfg) -> nn.Module:
    """
    带向后兼容性的损失函数创建器
    仅在需要兼容旧配置文件时使用
    """
    dice_weight = getattr(cfg, 'dice_weight', 0.3)
    
    # 兼容focal_weight
    focal_weight = getattr(cfg, 'focal_weight', None)
    if focal_weight is None:
        focal_weight = getattr(cfg, 'boundary_weight', 0.3)  # 旧配置可能误用了boundary_weight
    
    # 兼容boundary_weight (原hausdorff_weight)
    boundary_weight = getattr(cfg, 'boundary_weight', None)
    if boundary_weight is None:
        boundary_weight = getattr(cfg, 'hausdorff_weight', 0.4)
    
    dice_smooth = getattr(cfg, 'dice_smooth', 1e-5)
    focal_alpha = getattr(cfg, 'focal_alpha', 1.0)
    focal_gamma = getattr(cfg, 'focal_gamma', 2.0)
    
    # 兼容boundary参数
    boundary_alpha = getattr(cfg, 'boundary_alpha', None)
    if boundary_alpha is None:
        hausdorff_sample_points = getattr(cfg, 'hausdorff_sample_points', 1000)
        boundary_alpha = min(2.0, max(0.5, hausdorff_sample_points / 1000.0))
    
    normalize_boundary = getattr(cfg, 'normalize_boundary', None)
    if normalize_boundary is None:
        normalize_boundary = getattr(cfg, 'normalize_hausdorff', True)
    
    # 动态权重参数
    use_dynamic_weights = getattr(cfg, 'use_dynamic_weights', False)
    total_epochs = getattr(cfg, 'total_epochs', 30)
    weight_stages = getattr(cfg, 'weight_stages', None)
    
    # 兼容权重阶段中的hausdorff键
    if weight_stages is not None:
        for stage in weight_stages:
            if 'weights' in stage and 'hausdorff' in stage['weights']:
                if 'boundary' not in stage['weights']:
                    stage['weights']['boundary'] = stage['weights']['hausdorff']
                del stage['weights']['hausdorff']
    
    print("Warning: Using compatibility mode for old configuration. Consider updating to new parameter names.")
    
    return UnifiedTripleLoss(
        dice_weight=dice_weight,
        focal_weight=focal_weight,
        boundary_weight=boundary_weight,
        dice_smooth=dice_smooth,
        focal_alpha=focal_alpha,
        focal_gamma=focal_gamma,
        boundary_alpha=boundary_alpha,
        normalize_boundary=normalize_boundary,
        use_dynamic_weights=use_dynamic_weights,
        total_epochs=total_epochs,
        weight_stages=weight_stages
    )

def extract_boundary_points_for_hd95(mask: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
    """
    Args:
        mask: [H, W] 二值mask
        threshold: 二值化阈值
        
    Returns:
        boundary_points: [N, 2] 边界点坐标 (y, x)
    """
    # 二值化
    binary_mask = (mask > threshold).float()
    
    # 使用形态学操作提取边界
    kernel = torch.ones(3, 3, device=mask.device, dtype=mask.dtype)
    
    # 膨胀和腐蚀
    mask_4d = binary_mask.unsqueeze(0).unsqueeze(0)
    dilated = F.conv2d(mask_4d, kernel.unsqueeze(0).unsqueeze(0), padding=1)
    dilated = (dilated > 0).float()
    
    eroded = -F.conv2d(-mask_4d, kernel.unsqueeze(0).unsqueeze(0), padding=1)
    eroded = (eroded > 8).float()  # 只有当9个像素都是1时才保留
    
    # 边界 = 膨胀 - 腐蚀
    boundary = dilated.squeeze() - eroded.squeeze()
    boundary = (boundary > 0).float()
    
    # 如果没有边界点，返回所有前景点
    if boundary.sum() == 0:
        boundary = binary_mask
    
    # 提取坐标
    y_coords, x_coords = torch.where(boundary > 0)
    if len(y_coords) == 0:
        # 如果仍然没有点，返回图像中心
        H, W = mask.shape
        return torch.tensor([[H//2, W//2]], device=mask.device, dtype=torch.float32)
    
    boundary_points = torch.stack([y_coords.float(), x_coords.float()], dim=1)
    return boundary_points



def compute_hd95(pred_mask: torch.Tensor, gt_mask: torch.Tensor, threshold: float = 0.5) -> float:
    """
    计算95th percentile Hausdorff Distance
    
    Args:
        pred_mask: [H, W] 预测mask (概率或二值)
        gt_mask: [H, W] 真实mask (二值)
        threshold: 二值化阈值
        
    Returns:
        hd95: HD95距离值
    """
    # 提取边界点
    pred_boundary = extract_boundary_points_for_hd95(pred_mask, threshold)
    gt_boundary = extract_boundary_points_for_hd95(gt_mask, threshold)
    
    # 如果任一mask为空，返回最大可能距离
    if pred_boundary.shape[0] == 0 or gt_boundary.shape[0] == 0:
        H, W = pred_mask.shape
        return float(np.sqrt(H*H + W*W))
    
    # 如果边界点相同（完美匹配），返回0
    if torch.equal(pred_boundary, gt_boundary):
        return 0.0
    
    # 计算距离矩阵
    pred_boundary = pred_boundary.float()
    gt_boundary = gt_boundary.float()
    
    # 计算从pred到gt的距离
    dist_pred_to_gt = torch.cdist(pred_boundary, gt_boundary, p=2)  # [N_pred, N_gt]
    min_dist_pred_to_gt = dist_pred_to_gt.min(dim=1)[0]  # [N_pred]
    
    # 计算从gt到pred的距离
    dist_gt_to_pred = torch.cdist(gt_boundary, pred_boundary, p=2)  # [N_gt, N_pred]
    min_dist_gt_to_pred = dist_gt_to_pred.min(dim=1)[0]  # [N_gt]
    
    # 合并所有距离
    all_distances = torch.cat([min_dist_pred_to_gt, min_dist_gt_to_pred])
    
    # 计算95th percentile
    if len(all_distances) == 0:
        return 0.0
    
    # 转换为numpy进行百分位数计算
    distances_np = all_distances.detach().cpu().numpy()
    hd95 = np.percentile(distances_np, 95)
    
    return float(hd95)


def compute_pixel_accuracy(pred_mask: torch.Tensor, gt_mask: torch.Tensor, threshold: float = 0.5) -> float:
    """
    计算像素精度 (Pixel Accuracy)
    
    Args:
        pred_mask: [H, W] 预测mask (概率或二值)
        gt_mask: [H, W] 真实mask (二值) 
        threshold: 二值化阈值
        
    Returns:
        pa: 像素精度
    """
    # 二值化预测
    pred_binary = (pred_mask > threshold).float()
    gt_binary = (gt_mask > threshold).float()
    
    # 计算正确预测的像素数
    correct_pixels = (pred_binary == gt_binary).float().sum()
    total_pixels = torch.numel(gt_binary)
    
    # 像素精度
    pa = correct_pixels / total_pixels
    return pa.item()


# 计算评估指标（接收概率或logits）- 增强版
def calculate_metrics(pred: torch.Tensor, 
                     gt_mask: torch.Tensor, 
                     threshold: float = 0.5,
                     input_is_logits: bool = True,
                     enable_hd95: bool = False) -> Dict[str, float]:
    """
    计算分割指标（增强版，包含PA和HD95）
    
    Args:
        pred: [H, W] 预测值 (logits或概率)
        gt_mask: [H, W] 真实标签
        threshold: 二值化阈值
        input_is_logits: 输入是否为logits
        compute_hd95: 是否计算HD95（计算量较大）
    
    Returns:
        metrics: 包含所有指标的字典
    """
    if input_is_logits:
        pred_probs = torch.sigmoid(pred)
    else:
        pred_probs = pred
    
    pred_binary = (pred_probs > threshold).float()
    gt_binary = (gt_mask > threshold).float()
    
    # 基础指标
    intersection = (pred_binary * gt_binary).sum()
    pred_sum = pred_binary.sum()
    gt_sum = gt_binary.sum()
    union = pred_sum + gt_sum - intersection
    
    # 避免除零
    epsilon = 1e-7
    
    iou = (intersection + epsilon) / (union + epsilon)
    dice = (2 * intersection + epsilon) / (pred_sum + gt_sum + epsilon)
    precision = (intersection + epsilon) / (pred_sum + epsilon)
    recall = (intersection + epsilon) / (gt_sum + epsilon)
    f1 = 2 * precision * recall / (precision + recall + epsilon)
    
    # 新增：像素精度 (PA)
    pa = compute_pixel_accuracy(pred_probs, gt_mask, threshold)
    
    # 基础指标字典
    metrics = {
        'iou': iou.item(),
        'dice': dice.item(),
        'precision': precision.item(),
        'recall': recall.item(),
        'f1': f1.item(),
        'pa': pa  # 新增：像素精度
    }
    
    # 新增：HD95（可选计算，因为计算量较大）
    if enable_hd95:
        try:
            hd95 = compute_hd95(pred_probs, gt_mask, threshold)
            metrics['hd95'] = hd95
        except Exception as e:
            # 如果HD95计算失败，设为默认值
            metrics['hd95'] = float('inf')
            print(f"Warning: HD95 calculation failed: {e}")
    else:
        metrics['hd95'] = None
    
    return metrics


def calculate_batch_metrics(pred_masks: torch.Tensor, 
                          gt_masks: torch.Tensor, 
                          threshold: float = 0.5,
                          input_is_logits: bool = True,
                          enable_hd95: bool = False) -> Dict[str, float]:
    """
    计算batch的平均指标（增强版，包含PA和HD95）
    
    Args:
        pred_masks: [B, H, W] 预测值 (logits或概率)
        gt_masks: [B, H, W] 真实标签
        threshold: 二值化阈值
        input_is_logits: 输入是否为logits
        compute_hd95: 是否计算HD95
    
    Returns:
        avg_metrics: 平均指标字典
    """
    batch_size = pred_masks.size(0)
    metrics_list = []
    
    for i in range(batch_size):
        metrics = calculate_metrics(
            pred_masks[i], 
            gt_masks[i], 
            threshold, 
            input_is_logits,
            compute_hd95
        )
        metrics_list.append(metrics)
    
    # 计算平均值
    avg_metrics = {}
    for key in metrics_list[0].keys():
        if key == 'hd95' and not compute_hd95:
            avg_metrics[key] = None
        else:
            valid_values = [m[key] for m in metrics_list 
                           if m[key] is not None and not np.isinf(m[key])]
            if valid_values:
                avg_metrics[key] = sum(valid_values) / len(valid_values)
            else:
                avg_metrics[key] = 0.0 if key != 'hd95' else float('inf')
    
    return avg_metrics


# 用于大批量验证的高效版本
def calculate_batch_metrics_fast(pred_masks: torch.Tensor, 
                               gt_masks: torch.Tensor, 
                               threshold: float = 0.5,
                               input_is_logits: bool = True) -> Dict[str, float]:
    """
    快速批量指标计算（不包含HD95，用于训练时监控）
    
    Args:
        pred_masks: [B, H, W] 预测值
        gt_masks: [B, H, W] 真实标签
        threshold: 二值化阈值
        input_is_logits: 输入是否为logits
    
    Returns:
        avg_metrics: 平均指标字典（不包含HD95）
    """
    if input_is_logits:
        pred_probs = torch.sigmoid(pred_masks)
    else:
        pred_probs = pred_masks
    
    pred_binary = (pred_probs > threshold).float()
    gt_binary = (gt_masks > threshold).float()
    
    # 批量计算基础指标
    batch_size = pred_masks.size(0)
    pred_flat = pred_binary.view(batch_size, -1)
    gt_flat = gt_binary.view(batch_size, -1)
    
    intersection = (pred_flat * gt_flat).sum(dim=1)
    pred_sum = pred_flat.sum(dim=1)
    gt_sum = gt_flat.sum(dim=1)
    union = pred_sum + gt_sum - intersection
    
    epsilon = 1e-7
    
    # 批量计算指标
    iou = (intersection + epsilon) / (union + epsilon)
    dice = (2 * intersection + epsilon) / (pred_sum + gt_sum + epsilon)
    precision = (intersection + epsilon) / (pred_sum + epsilon)
    recall = (intersection + epsilon) / (gt_sum + epsilon)
    f1 = 2 * precision * recall / (precision + recall + epsilon)
    
    # 批量计算像素精度
    correct_pixels = (pred_binary == gt_binary).float().view(batch_size, -1).sum(dim=1)
    total_pixels = gt_binary.view(batch_size, -1).shape[1]
    pa = correct_pixels / total_pixels
    
    return {
        'iou': iou.mean().item(),
        'dice': dice.mean().item(),
        'precision': precision.mean().item(),
        'recall': recall.mean().item(),
        'f1': f1.mean().item(),
        'pa': pa.mean().item(),
        'hd95': None  # 快速版本不计算HD95
    }


# 用于详细分析的完整版本
def calculate_detailed_metrics(pred_masks: torch.Tensor, 
                             gt_masks: torch.Tensor, 
                             threshold_list: List[float] = [0.3, 0.4, 0.5, 0.6, 0.7],
                             input_is_logits: bool = True) -> Dict[str, Dict[str, float]]:
    """
    计算多个阈值下的详细指标（用于模型分析）
    
    Args:
        pred_masks: [B, H, W] 预测值
        gt_masks: [B, H, W] 真实标签
        threshold_list: 阈值列表
        input_is_logits: 输入是否为logits
    
    Returns:
        detailed_metrics: 多阈值指标字典
    """
    detailed_metrics = {}
    
    for threshold in threshold_list:
        metrics = calculate_batch_metrics(
            pred_masks, gt_masks, 
            threshold=threshold, 
            input_is_logits=input_is_logits,
            compute_hd95=True  # 详细分析时计算HD95
        )
        detailed_metrics[f'threshold_{threshold}'] = metrics
    
    # 找到最佳阈值
    best_threshold = None
    best_dice = 0.0
    for thresh_key, metrics in detailed_metrics.items():
        if metrics['dice'] > best_dice:
            best_dice = metrics['dice']
            best_threshold = thresh_key
    
    detailed_metrics['best_threshold'] = best_threshold
    detailed_metrics['best_metrics'] = detailed_metrics[best_threshold] if best_threshold else None
    
    return detailed_metrics


# 向后兼容的接口
def calculate_metrics_with_pa_hd95(pred: torch.Tensor, 
                                 gt_mask: torch.Tensor, 
                                 threshold: float = 0.5,
                                 input_is_logits: bool = True) -> Dict[str, float]:
    """向后兼容的接口，包含PA和HD95"""
    return calculate_metrics(pred, gt_mask, threshold, input_is_logits, compute_hd95=True)