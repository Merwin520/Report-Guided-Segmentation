...
class KernelSpatialModulation_Global(nn.Module):
    ...
    def get_channel_attention(self, x):
        if self.act_type == 'sigmoid':
            ...
            # GELU激活函数，输出范围调整为与其他激活函数一致 [0, 2]
            gelu_out = F.gelu(self.channel_fc(x).view(x.size(0), 1, 1, -1, x.size(-2), x.size(
                -1)) / self.temperature)
            channel_attention = torch.sigmoid(gelu_out) * self.att_multi
        else:
            raise NotImplementedError(f"Activation type {self.act_type} not supported")
    ...
    def get_filter_attention(self, x):
        if self.act_type == 'sigmoid':
            ...
            gelu_out = F.gelu(self.filter_fc(x).view(x.size(0), 1, -1, 1, x.size(-2), x.size(
                -1)) / self.temperature)
            filter_attention = torch.sigmoid(gelu_out) * self.att_multi
        else:
            raise NotImplementedError(f"Activation type {self.act_type} not supported")
    ...
    def get_spatial_attention(self, x):
        ...
        if self.act_type == 'sigmoid':
            ...
            gelu_out = F.gelu(spatial_attention / self.temperature)
            spatial_attention = torch.sigmoid(gelu_out) * self.att_multi
        else:
            raise NotImplementedError(f"Activation type {self.act_type} not supported")
    ...
    def get_kernel_attention(self, x):
        ...
        if self.act_type == 'softmax':
            ...
            # GELU用于kernel attention时使用softmax归一化以确保权重和为1
            kernel_attention = F.softmax(F.gelu(kernel_attention / self.kernel_temp), dim=1)
        else:
            raise NotImplementedError(f"Activation type {self.act_type} not supported")
...
class FrequencyBandModulation(nn.Module):
    ...
    def sp_act(self, freq_weight):
        ...
        if self.act == 'sigmoid':
            ...
            # GELU激活函数，通过sigmoid确保输出范围一致性
            freq_weight = torch.sigmoid(F.gelu(freq_weight)) * 2
        else:
            raise NotImplementedError(f"Activation type {self.act} not supported")
        return freq_weight
...
class FDConv(nn.Conv2d):
    def __init__(self,
        ...
        # 验证激活函数类型，现在支持GELU
        assert self.ksm_local_act in ['sigmoid', 'tanh', 'gelu']
        assert self.ksm_global_act in ['softmax', 'sigmoid', 'tanh', 'gelu']
    ...
    def convert2dftweight(self, convert_param):
        ...
        # 修复：确保频域点数能被kernel_num整除
        total_freq_points = freq_indices.size(1)
        if self.kernel_num > 1:
            points_per_kernel = total_freq_points // self.kernel_num
            if points_per_kernel == 0:
                # 如果频域点数少于kernel_num，强制设为1
                self.kernel_num = 1
                points_per_kernel = total_freq_points
            else:
                # 只使用能被kernel_num整除的频域点
                usable_points = points_per_kernel * self.kernel_num
                freq_indices = freq_indices[:, :usable_points]
        else:
            points_per_kernel = total_freq_points
        ...
        if self.param_reduction < 1:
            # 随机选择频域点进行参数减少
            selected_points = int(freq_indices.size(1) * self.param_reduction)
            if selected_points > 0:
                perm_indices = torch.randperm(freq_indices.size(1), 
                                            generator=torch.Generator().manual_seed(freq_indices.size(1)))
                freq_indices = freq_indices[:, perm_indices[:selected_points]]
        ...
        # 修复：为每个param_ratio生成对应的indices
        self.indices = []
        current_freq_indices = freq_indices  # 使用处理后的freq_indices
   
        for i in range(self.param_ratio):
            if self.kernel_num > 1:
                # 确保能正确reshape
                total_points = current_freq_indices.size(1)
                points_per_kernel = total_points // self.kernel_num
                usable_points = points_per_kernel * self.kernel_num
           
                if usable_points > 0:
                    # 只使用前usable_points个点
                    indices_for_reshape = current_freq_indices[:, :usable_points]
                    reshaped_indices = indices_for_reshape.reshape(2, self.kernel_num, points_per_kernel)
                else:
                    # 如果没有足够的点，创建一个最小的indices
                    reshaped_indices = current_freq_indices[:, :1].reshape(2, 1, 1)
            else:
                # kernel_num = 1的情况
                reshaped_indices = current_freq_indices.reshape(2, 1, -1)
           
            self.indices.append(reshaped_indices)
    ...
    def forward(self, x):
        ...
        #channel_attention, filter_attention, spatial_attention, kernel_attention = self.KSM_Global(global_x)
        if global_x.size(0) == 1:
            # 临时用 eval() 模式，禁用 BN 训练行为
        ...
        if self.use_ksm_local:
            ...
            elif self.ksm_local_act == 'gelu':
                gelu_out = F.gelu(hr_att_logit)
                hr_att = torch.sigmoid(gelu_out) * self.att_multi
            else:
                raise NotImplementedError(f"Local activation type {self.ksm_local_act} not supported")
        else:
            hr_att = 1
        b = x.size(0)
        ...
        DFT_map = torch.zeros(
            ...
            device=x.device)
        if isinstance(kernel_attention, float):
        # 如果kernel_attention是float（来自skip函数），转换为tensor
            kernel_attention = torch.ones(b, self.param_ratio, self.kernel_num, 1, 
                                    device=x.device, dtype=x.dtype) * kernel_attention
        else:
