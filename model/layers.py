...
from .CoordGate import CoordGate
...
class Neck(nn.Module):
    def __init__(self,
        ...
        self.coordconv = nn.Sequential(
            CoordGateConv(out_channels[1], out_channels[1], 3, 1),
...
class CoordGateConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, stride=1):
        ...
        self.target_size = (26, 26)
        
        self.coord_gate = CoordGate(
        ...
        if kernel_size != 1 or stride != 1 or in_channels != out_channels:
    ...
    def forward(self, x):
        ...
        if (h, w) != (target_h, target_w):
            ...
            x_gated = self.coord_gate(x_resized)
            x_gated = F.interpolate(x_gated, size=(h, w), mode='bilinear', align_corners=False)
        else:
            x_gated = self.coord_gate(x)

        if self.conv is not None:
            x_gated = self.conv(x_gated)
        
        return x_gated

class OriginalCoordConv(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size=3,
